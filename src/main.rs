...
fn main() {
  ...
  let ply = GameObject::new(2, Position::new(AREA_MAX_X / 2, AREA_MAX_Y / 2, 1)).create_ref();
  ply.borrow_mut().set_area(&area);
  {
    let mut area = area.borrow_mut();
    for x in 0..AREA_MAX_X {
      for y in 0..AREA_MAX_Y {
        let obj = GameObject::new(0, Position::new(x, y, 0)).create_ref();
        area.insert(obj).expect("Successful adding");
        if (y == 0 || y == AREA_MAX_Y - 1) && x % 2 == 0 || (x == 0 || x == AREA_MAX_X - 1) && y % 2 == 0 {
          let nish = GameObject::new(1, Position::new(x, y, 1)).create_ref();
          nishals.push(Rc::clone(&nish));
          area.insert(nish).expect("Successful adding");
        }
      }
    }
    players.push(Rc::clone(&ply));
    area.insert(Rc::clone(&ply));
  }
  ...
  loop {
    ...
    {
      let mut player = ply.borrow_mut();
      let Position {x: cx , y: cy, z: cz} = player.get_pos();
